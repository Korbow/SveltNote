...
function Cart({ cart, updateCart }) {
	...
	const nombre = cart.reduce(
		(acc, plantType) => acc + plantType.amount,
		0
	)
	useEffect(() => {
		document.title = `SveltNote - Shop: ${total}€ `
	}, [total])
	...
	return isOpen ? (
		<div id='cart_drawer'>
			<div className='lmj-cart'>
				...
				{cart.length > 0 ? (
					<div>
						...
						<ul className="ItemCart">
							{cart.map(({ name, price, cover, amount }, index) => (
								<div className="CartSection"  key={`${name}-${index}`}>
									<div>
										<img className='imgCartSelection' src={cover} alt={`${name} cover`} />
									</div>
									<div className="NameCartSelection">
									{name} <span> &nbsp;&nbsp; x{amount} </span>
									</div>
									<div className="PriceCartSelection">
										{price}€  
									</div>
									<div className="AmountCartSelection">
									</div>
								</div>
							))}
						...
						<button onClick={() => {
							...
							const cartAlignText = document.getElementById('cart_drawer')

							if (!cartAlignText) return; 
							if (cartAlignText.classList.contains('align')){
								cartAlignText.classList.remove("align");
							}
						
						}
						}
							 class="ctaEmptyCart">
							<div className="alignSvgText">
								<span className="hover-underline-animation"> Empty the Cart 
					...
					<div className="NoSelection">No selection</div>
				)}
			</div>
		</div>
	) : (
		<div className='lmj-cart-closed'>
			<button
				className='lmj-cart-toggle-button2'
				onClick={() => setIsOpen(true)}
			>
				<svg className="cart" fill="white" viewBox="0 0 576 512" height="1em" xmlns="http://www.w3.org/2000/svg"><path d="M0 24C0 10.7 10.7 0 24 0H69.5c22 0 41.5 12.8 50.6 32h411c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3H170.7l5.4 28.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24 10.7 24 24s-10.7 24-24 24H199.7c-34.6 0-64.3-24.6-70.7-58.5L77.4 54.5c-.7-3.8-4-6.5-7.9-6.5H24C10.7 48 0 37.3 0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"></path></svg>
				<p className="text_OP_cart">OPEN CART</p>
				{nombre > 0 ? ( <span className="notif-dot">{nombre}</span> 
				):(
					<span></span>	
				)}
			</button>
			
			<svg className="ArrowSVG" width="50px" height="50px" viewBox="0 0 25 25"  fill="black" xmlns="http://www.w3.org/2000/svg">
				<g>
